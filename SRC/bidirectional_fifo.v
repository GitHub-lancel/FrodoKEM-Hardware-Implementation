...
module bidirectional_fifo(
    ...
    always @(posedge clk or negedge rstn) begin
        if(~rstn) begin
            ...
            // -------- 64->448 新逻辑：连续输入 + 可flush --------
                ...
                if(wr_en_64) begin
                    // 64->448：第1个64bit放在最高64位，第2个放在次高64位，
                    // 提前flush时保持低位补0；仅改变数据排布，不改变控制时序
                    case(wr_cnt)
                        3'd0: buffer <= {data_64_in, 384'b0};
                        3'd1: buffer <= {buffer[447:384], data_64_in, 320'b0};
                        3'd2: buffer <= {buffer[447:320], data_64_in, 256'b0};
                        3'd3: buffer <= {buffer[447:256], data_64_in, 192'b0};
                        3'd4: buffer <= {buffer[447:192], data_64_in, 128'b0};
                        3'd5: buffer <= {buffer[447:128], data_64_in,  64'b0};
                        default: buffer <= {buffer[447:64], data_64_in};
                    endcase
    ...
    */
    assign data_448_out_val = (mode == 1'b1 && NS == OUTPUT);
endmodule
