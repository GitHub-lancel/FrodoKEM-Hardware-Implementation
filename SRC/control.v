...
//=====================================================================整体用到的控制信号================================================================
    ...
    assign addr_run_cnt_clr = ((substate == MAC0) && (substate_nxt == ABS)) || DONE_r;//用于给解码器传信号，在矩阵运算中让loopmax回归正常值
...
//=====================================================================H:hash的控制=======================================================================
    ...
    assign lastmsg_val = (opcode == 4'b1000 || opcode == 4'b1001)  ?  (msg_val && loopafull_r_r)   : (addr_cnt == rd_num) && loopafull_r_r && msg_val; //区分在矩阵运算下的数据
...
//=====================================================================encode的控制=================================================================
    ...
    assign encode_msg_val = substate == MAT64 ? addrd_add_r_r : 1'b0;
...
//====================================================================数据搬运指令补丁
    always@(posedge clk or negedge  rstn)begin
            if(!rstn)
                ...
                twostate <= 1'b0;
            else if(encode_done && opcode == 4'b1111)
                twostate <= 1'b1;
            else
                twostate <= twostate;     
        end
...
//=====================================================================输出valid==============
    ...
    always@(posedge clk or negedge rstn)begin
        if(!rstn)
            ...
            DO_valid <= addra_add_r_r && (opcode == 4'b1101);    
    end

//=====================================================================地址的控制=================================================================

    assign ADDR_clra = (opcode == 4'b1000 || opcode == 4'b1001) ? (substate == RUN) || (substate == DONE)   : (addr_load || substate == DONE);//增加判断opcode，当用于矩阵运算,load无信号，单独定义在RUN中让信号拉高
